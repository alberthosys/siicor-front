...
export class AclComponent implements OnInit {
  ...
  constructor(
    ...
    this.formAclEstandar = formBuilder.group({
      ...
      mask: [null, Validators.compose([Validators.pattern('((^|\\.)((25[0-5]_*)|(2[0-4]\\d_*)|(1\\d\\d_*)|([1-9]?\\d_*))){4}_*$')])],
  ...
  public clear() {
    ...
    this.formAclEstandar.controls.mask.setValue(null);
  ...
  guardar() {
    ...
    if (this.formAclEstandar.valid) {
      let mask = this.formAclEstandar.controls.mask.value;
      if (mask) {
        mask = mask.replace(' ', '');
        mask = mask.trim();
      }
